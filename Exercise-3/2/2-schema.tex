...
\begin{document}
	...
	\scalebox{.87}{
		...
		\node[relationship] (assigned) [below right=of article] {categorized} edge (article);
		...
		\node[relationship] (subs) [right=4cm of article] {subcategorized}  edge (article) edge (cat);
		...
		\coordinate[right=of article, label={*}]; 
		\coordinate[below=of subs, label={*}]; 
